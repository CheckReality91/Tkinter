...
def check_email():
    if reg_email.get() == retype_email.get():
        return True
    elif '@' not in reg_email.get():
        messagebox.showerror(title='Error', message='Not a valid E-mail.')
    else:
...
def register_window():
    ...
    register_btn = Button(register_frame, text='Register', command=register_user).grid(column=1, row=3, pady=10)
    cancel_btn = Button(register_frame, text="Cancel", command=window.destroy).grid(column=2, row=3, pady=10)
...
cancel_btn = Button(frame, text='Cancel', command=root.destroy).grid(column=2, row=2,pady=10)
